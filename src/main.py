...
def main(config):
    ...
    log_dir = f"runs/{config.num_classes}_{config.num_shot}_{config.random_seed}_{config.learning_rate}_{config.hidden_dim}_{config.num_layers}"
    ...
    model = MANN(config.num_classes, config.num_shot + 1, config.hidden_dim, config.num_layers)
    ...
    for step in range(config.train_steps):
        ...
        writer.add_scalar("Learning Rate", config.learning_rate, step)
        times.append([t1 - t0, t2 - t1])

        with torch.no_grad():
            pred = torch.reshape(pred, [-1, config.num_shot + 1, config.num_classes, config.num_classes])
            train_pred = torch.argmax(pred[:, -1, :, :], axis=2)
            train_labels = torch.argmax(l[:, -1, :, :], axis=2)
            train_acc = train_pred.eq(train_labels).sum().item() / (config.meta_batch_size * config.num_classes)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_layers", type=int, default=1)  # New argument
